...
public class Client{
    ...
    public static void employeeOperations(ObjectOutputStream outs, ObjectInputStream ins) throws IOException {    
        ...
        try {
            ...
            do{    
                ...
                System.out.print("\n a. Signup \n b. Login \n  --> ");
                cho = sc.next();
                System.out.println("Your option : " + cho);
                cho = cho.toLowerCase();
            ...
            do{
                ...
                System.out.println("\n 1. Apply for a leave");              // DONE
                System.out.println(" 2. View your Leave History");          // DONE
                System.out.println(" 3. Check your Leave Balance");         // DONE
                System.out.println(" 4. Logout");                           // DONE
                System.out.print("\nEnter your choice : ");
                chc = sc.nextInt();
                System.out.println("Your option : " + chc);
                ...
                switch (chc) {
                    ...
                    case 2:
                        outs.reset();
                        outs.writeObject(emp_ID);
                        LinkedList<Leave> ll = (LinkedList<Leave>)ins.readObject();
                        if(ll.size() == 0)   System.out.println("\n  **** NO LEAVE HISTORY ****");
                        else                      emp.LeaveHistory(ll);
                        break;
                    case 3:
                        emp.empLeaveBalance();
                        break;
    ...
    public static void managerOperations(ObjectOutputStream outs, ObjectInputStream ins) {    
        ...
        try {
            ...
            do{
                ...
                switch (chc) {
                    case 1:
                        ll = (LinkedList<Leave>)ins.readObject();
                        if(ll.size() == 0)      System.out.println("\n --> No new leaves present !! ");
                        else                    mng.mngrNewLeaves(ll);
                        break;
                    ...
                    case 2:   
                        ...
                        if(ll.size() == 0)   System.out.println("\n --> All leaves have been managed !!");
                            ...
                            mng.mngrNewLeaves(ll);
                            Leave lv = mng.mngrEditLeaves(ll);
                            outs.writeObject(lv);    
                        }
                        break;
                    case 3:
                    ...
                    case 4:
                        ...
                        do{
                            ...
                            if(id > -1){
                                ll = (LinkedList<Leave>)ins.readObject();
                                if(ll == null)     System.out.println("\n   *-*-*-* Requested employee doesn't exist *-*-*-*");
                                else{
                                    int size = ll.size();
                                    Leave l = new Leave();
                                    if(size > 0)    l.displayLeaves(ll);
    ...
    public static void displayLeaveReport(HashMap<String, HashMap<String, String>> hm) {
        ...
        String leftAlignFormat = " %-6s | %-2s | %-10s | %-7s | %-7s | %-13s | %-10s |\n";
        System.out.println("        +----+------------+---------+---------+---------------+------------+");
        System.out.format(leftAlignFormat, "","ID", "USERNAME","NAME", "#USED", "#AVAILABLE", "#TOTAL");
        System.out.println("        +----+------------+---------+---------+---------------+------------+");
        int i = 1;

        for(String key : hm.keySet()){
            System.out.format(leftAlignFormat, "",hm.get(key).get("ID") , key ,hm.get(key).get("Name") , hm.get(key).get("Used"), hm.get(key).get("Available"), hm.get(key).get("Allowed"));
            i++;
        }
        System.out.println("        +----+------------+---------+---------+---------------+------------+");
    }
